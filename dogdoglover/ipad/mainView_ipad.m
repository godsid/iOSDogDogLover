...
@interface mainView_ipad ()

@end

@implementation mainView_ipad
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        if([self.tableElementsTitle count]>0)
        {
            static NSString *sampleIndentifier = @"CustomMain_ipad";
            CustomMain_ipad *cell = (CustomMain_ipad*)[tableView dequeueReusableCellWithIdentifier:sampleIndentifier];
            if (cell == nil)
            {
                cell = [[CustomMain_ipad alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:sampleIndentifier];
                NSArray *topLevelObjects = [[NSBundle mainBundle] loadNibNamed:@"CustomMain_ipad" owner:nil options:nil];
                
                for(id currentObject in topLevelObjects)
                {
                    if([currentObject isKindOfClass:[UITableViewCell class]])
                    {
                        cell  = (CustomMain_ipad*)currentObject;
                        break;
                    }
                }
            }
